...
const RoomCard = ({ roomNumber, roomType, roomStatus, roomAvailable, onViewDetails, onUpdateDetails, onTenantBillDetails }) => {
            ...
            <div className="absolute inset-0 bg-black bg-opacity-50 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                <div className="flex space-x-4 p-4">
                    ...
                    {roomStatus === 'occupied' && (
                        <div 
                            className="p-2 bg-gray-800 rounded-lg hover:bg-gray-600 transition-colors"
                            onClick={onTenantBillDetails}
                        >
                            <HiOutlineClipboardList className="text-white" size={24} />
                        </div>
                    )}
                </div>
            </div>
